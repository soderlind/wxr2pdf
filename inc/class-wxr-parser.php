...
//	public $error_message = '';
	...
	function parse( $file, $post_type = 'post' ) {
		...
		$regexp_parser = new WXR2PDF_WXR_Parser_Regex;
...
class WXR2PDF_WXR_Parser_SimpleXML {
	function parse( $file, $post_type ) {
		...
		$site_title = $xml->xpath('/rss/channel/title');
		$site_title = (string) trim( $site_title[0] );

		$site_decription = $xml->xpath('/rss/channel/description');
		$site_decription = (string) trim( $site_decription[0] );

		$language = $xml->xpath('/rss/channel/language');
		$language = (string) str_replace('-', '_', trim( $language[0] ) );
		...
		return array(
			...
			'site_decription' => $site_decription,
			'language'        => $language,
			'version'         => $wxr_version
		);
	}
...
class WXR2PDF_WXR_Parser_Regex {
	...
	public function parse($file, $post_type){
